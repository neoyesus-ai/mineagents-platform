...
const toAgent = (row: Row): AgentRecord => ({
  id: String(row.id),
...
const toTask = (row: Row): TaskRecord => ({
  id: String(row.id),
...
export class CoordinatorStore {
  ...
  listAgents(): AgentRecord[] {
    const rows = this.database.prepare("SELECT * FROM agents ORDER BY updated_at DESC, created_at DESC").all() as Row[];
    return rows.map(toAgent);
  }

  upsertAgentHeartbeat(input: HeartbeatInput): AgentRecord {
    const timestamp = now();
    const id = input.id?.trim() || randomUUID();
    const existing = this.database.prepare("SELECT id FROM agents WHERE id = :id").get({ id }) as
      | Row
      | undefined;
    ...
    if (existing) {
      ...
      this.database.prepare(`
        INSERT INTO agents (id, name, role, status, last_heartbeat_at, created_at, updated_at)
        VALUES (:id, :name, :role, 'online', :lastHeartbeatAt, :createdAt, :updatedAt)
      `).run({
        id,
        name: input.name,
        role: input.role ?? null,
  ...
  getAgentById(id: string): AgentRecord {
    const row = this.database.prepare("SELECT * FROM agents WHERE id = :id").get({ id }) as Row | undefined;
    if (!row) {
  ...
  listProjects(): ProjectRecord[] {
    const rows = this.database.prepare("SELECT * FROM projects ORDER BY updated_at DESC, created_at DESC").all() as Row[];
  ...
  getProjectById(id: string): ProjectRecord {
    const row = this.database.prepare("SELECT * FROM projects WHERE id = :id").get({ id }) as Row | undefined;
    if (!row) {
  ...
  listTasks(): TaskRecord[] {
    const rows = this.database.prepare("SELECT * FROM tasks ORDER BY created_at DESC, updated_at DESC").all() as Row[];
  ...
  createTask(input: TaskCreateInput): TaskRecord {
    ...
    this.database.prepare(`
      INSERT INTO tasks (
        ...
        failure_reason, created_at, updated_at, started_at, completed_at, failed_at, cancelled_at
      )
      VALUES (
        :id, :projectId, :title, :description, 'pending', NULL,
        NULL, :createdAt, :updatedAt, NULL, NULL, NULL, NULL
      )
    `).run({
      id,
  ...
  getTaskById(id: string): TaskRecord {
    const row = this.database.prepare("SELECT * FROM tasks WHERE id = :id").get({ id }) as Row | undefined;
    if (!row) {
  ...
  claimNextTask(agentId: string): TaskRecord | null {
  ...
  patchTask(id: string, input: TaskPatchInput): TaskRecord {
    ...
    if (input.assignedAgentId !== undefined && input.assignedAgentId !== null) {
      this.getAgentById(input.assignedAgentId);
    }
    ...
    const cancelledAt = nextStatus === 'cancelled' ? timestamp : current.cancelledAt;
